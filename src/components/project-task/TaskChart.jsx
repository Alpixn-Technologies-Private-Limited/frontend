...
const TaskChart = () => {
  ...
  const taskAnnotations = {
    1: { text: "Milestones", color: "text-green-600 dark:text-green-400", position: "top" },
    2: { text: "blocked by client feedback", color: "text-green-400", position: "top" },
    4: { text: "Depend on UI Completion", color: "text-yellow-600 dark:text-yellow-400", position: "top" },
    6: { text: "Milestones", color: "text-red-600 dark:text-red-400", position: "top" },
    8: { text: "Revising Screen based on feedback", color: "text-green-600 dark:text-green-400", position: "top" },
    9: { text: "Checklist & final files pending", color: "text-yellow-600 dark:text-yellow-400", position: "top" },
    10: { text: "File milestone, include insights log", color: "text-red-600 dark:text-red-400", position: "top" },
  };

  return (
    <div className="flex-1 bg-white/60 dark:bg-white/10 backdrop-blur-lg transition-all">
      {/* Month Headers */}
      <div className="flex w-full border-b border-gray-300 dark:border-white/20 bg-white/40 dark:bg-white/5 sticky top-0 z-10 backdrop-blur-md">
        {monthBlocks.map(({ month, from, to }) => {
          ...
          return (
            <div
              key={month}
              className="text-lg font-bold text-gray-800 dark:text-gray-100 py-2 border-r border-gray-300 dark:border-white/20 text-center"
      ...
      {/* Day Headers */}
      <div className="flex text-[10px] text-gray-600 dark:text-gray-300 border-b border-gray-300 dark:border-white/10">
        {dailyHeaders.map((date, i) => {
          ...
          return (
            <div
              key={i}
              className={`text-center py-1 border-r border-gray-200 dark:border-white/10 flex flex-col items-center justify-center
                ${isSunday ? "bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400 font-semibold" : ""}
                ${((i + 1) % 3 === 0 && !isSunday) ? "font-semibold text-black dark:text-white" : ""}`}
              style={{ flexBasis: `${width}%`, minWidth: `${width}%` }}
            >
      ...
      <div className="relative z-0">
        ...
        {dailyHeaders.map((_, idx) => (
          <div
            key={idx}
            className="absolute top-0 bottom-0 w-[1px] bg-gray-200 dark:bg-white/10 z-0"
        ...
        {staticTasks.map((task) => {
          ...
          return (
            <div key={task.id} className="relative h-12 border-b border-gray-200 dark:border-white/5 flex items-center">
              {/* Bar */}
              <div
                className={`absolute h-6 rounded-full px-2 flex items-center text-white text-xs font-medium z-10 truncate 
                  transition-all duration-300 ease-in-out transform hover:scale-[1.03] hover:brightness-110 ${barColor}`}
                style={{
                  left: `${leftPercent}%`,
                  width: `${widthPercent}%`,
